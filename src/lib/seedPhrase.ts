// Complete BIP39 word list for secure seed phrases
export const BIP39_WORDS = [
  ...
  'nasty', 'nation', 'nature', 'near', 'neck', 'need', 'negative', 'neglect', 'neighbor', 'nephew',
  ...
  'orbit', 'order', 'ordinary', 'organ', 'orient', 'original', 'orphan', 'ostrich', 'other', 'outdoor',
...
// Utility functions for seed phrase handling
export const generateSecureSeedPhrase = (): string[] => {
  ...
  for (let i = 0; i < 12; i++) {
    const randomIndex = randomArray[i] % BIP39_WORDS.length;
...
export const getSeedPhraseStrength = (phrase: string[]): {
  ...
  if (phrase.length === 12) {
    ...
    feedback.push('Must be exactly 12 words');
  }
  
  // Check all words are valid BIP39 words
  const validWords = phrase.filter(word => {
    const trimmedWord = word.trim().toLowerCase();
    return BIP39_WORDS.includes(trimmedWord);
  });
  
  if (validWords.length === 12) {
    score += 40;
  } else {
    feedback.push(`${12 - validWords.length} invalid words detected`);
  }
  
  // Check for duplicates
  const uniqueWords = new Set(phrase.map(w => w.trim().toLowerCase()));
  ...
  if (!hasConsecutiveWords) {
    ...
    feedback.push('Avoid consecutive words from the word list');
  }
