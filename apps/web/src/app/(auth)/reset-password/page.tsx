...
export default function ResetPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) setError(error.message); else setMessage('Password reset email sent if account exists.');
    setLoading(false);
  };
    ...
    <div className="min-h-screen flex items-center justify-center px-4 bg-gray-50">
      <div className="w-full max-w-md space-y-6 bg-white p-8 rounded shadow">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button disabled={loading} className="w-full bg-indigo-600 text-white py-2 rounded hover:bg-indigo-700 disabled:opacity-50">{loading ? 'Sending...' : 'Send reset link'}</button>
        </form>
